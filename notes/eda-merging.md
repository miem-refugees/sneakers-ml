...
- Пока работаем с датасетом с брендами. Взяли эмбеддинги `resnet-embedding-eda.ipynb` всех картинок c помощью `resnet152` и использовали SVD, PCA, TSNE.
Посмотрели как картинки лежат в пространстве.
- Также попробовали считать косинусное расстояние между картинками, чтобы найти дубликаты. Сработало неочень. Даже при большом
